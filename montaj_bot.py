...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "continue":
        ...
        # Выбор регистратора по количеству камер
        ...
        msg = (
            f"**Смета оборудования и работ:**\n"
            f"Камеры {cam_count} шт × {cam_price}₽ = {cameras_total}₽\n"
            f"Видеорегистратор = {reg_price}₽\n"
            f"HDD {hdd} = {hdd_price}₽\n"
            f"PoE-коммутаторы {switches} × {POE_PRICE}₽ = {switches_total}₽\n"
            f"Кабель {cable} м × {CABLE_PRICE}₽ = {cable_total}₽\n\n"
            f"**Монтаж:**\n"
            f"Установка камер: {cam_count} × {INSTALL_CAMERA}₽\n"
            f"Настройка регистратора: {CONFIG_DVR}₽\n"
            f"Пусконаладочные: {STARTUP_WORK}₽\n"
            f"Прокладка кабеля: {cable} × {INSTALL_CABLE}₽\n"
            f"Установка коммутаторов: {switches} × {INSTALL_SWITCH}₽\n\n"
            f"**ИТОГО: {grand_total}₽**"
        )
